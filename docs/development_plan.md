...
> **Current Phase:** Phase 6 — Hardware Acceleration & Optimization  
> **Overall Status:** Phase 3 complete ✓  Phase 4.0 complete ✓  Phase 4.1 complete ✓  Phase 4.2 complete ✓  Phase 4.3 complete ✓  Phase 4.4 complete ✓  Phase 4.5 complete ✓  Phase 4.6 complete ✓  Phase 4.7 complete ✓  Phase 5.1 in progress 🔧  Phase 5.2 in progress 🔧  Phase 5.3 complete ✓  Phase 5.4 in progress 🔧  Phase 5.5 code complete 🔧  Phase 5.6 complete ✓  Phase 5.7 complete ✓  Phase 6.1 in progress 🔧
...
- [x] **5.1.13** Integration test: boot → DHCP lease → `ping 10.0.2.2` → `wget 10.0.2.2:80/`
...
## Phase 5.7 — Lumina Mono Font Identity

**Status:** Complete ✓  
**Goal:** Replace the VGA 8×8 CP437 bitmap font with a visually cohesive 8×16 identity font
that matches the Lumina glassmorphism aesthetic.

### Design

Rather than embedding a new glyph set (Terminus/Spleen, ~4KB of hand-crafted hex) or adding
stb_truetype infrastructure (alpha-blending, glyph cache, TTF embedding), the chosen approach
stretches the existing high-quality CP437 data vertically via a 2× row-doubling formula:

```
drow 0       → glyph[0]             (cap/top pixel row)
drow 1..14   → glyph[(drow+1) >> 1] (each source row doubled)
drow 15      → 0x00                 (blank breathing room at baseline)
```

This is named **"Lumina Mono 8×16"** — same glyph art, proportions that read as a modern
monospace, with no new dependencies and zero additional binary size.

### Changes

| File | Change |
|------|--------|
| `kernel/include/drivers/video/font.h` | `FONT_H` 8 → 16; renamed identity to "Lumina Mono" |
| `kernel/drivers/video/font.c` | `font_draw_char` rewired to render 16 display rows via stretch formula |
| `kernel/drivers/video/fb_console.c` | `CON_ROWS` 90 → 45 (720 / 16) |
| `userspace/lib/libaether/gfx.c` | `FONT_H` 8 → 16 |
| `userspace/lib/include/widget.h` | `WGT_FONT_H` 8 → 16; cascades to all libwidget consumers |
| `userspace/lib/include/gfx.h` | Comment updated: "8×16 Lumina Mono" |
| `userspace/apps/init/main.c` | `FONT_H` 8 → 16 |
| `userspace/apps/statusbar/main.c` | `FONT_H` 8 → 16; `BOTBAR_Y` 744 → 696; `TERM_ROWS` added; `SIDE_H` 280 → 360 |
| `userspace/apps/aether_term/main.c` | `FONT_H` 8 → 16; `BOTBAR_Y` 744 → 696; `TERM_ROWS` 70 → 34 |

### Tasks

- [x] **5.7.1** Update kernel `font_draw_char` — 16-row 2× stretch formula
- [x] **5.7.2** Cascade `FONT_H=16` through kernel console, userspace gfx, and libwidget
- [x] **5.7.3** Recalculate `TERM_ROWS` (70 → 34) to fit 16px rows in 602px available height
- [x] **5.7.4** Fix `BOTBAR_Y` leftover bug (744 → 696) in statusbar and aether_term
- [x] **5.7.5** Adjust `SIDE_H` in statusbar (280 → 360) for taller 16px content

**Phase 5.7 Success Criteria:** All apps render 8×16 text with no layout overflow.
...
- [ ] **6.1.3** Port Mesa Gallium V3D driver (userspace)
  - **Prerequisites:** Stable V3D kernel driver (6.1.1 ✓), CL/BO ioctls exposed via syscalls
  - **Approach:** Cross-compile `mesa/src/gallium/drivers/v3d` for AArch64 bare-metal
    - Strip DRM/KMS layer — replace with AetherOS `gpu_*` syscall shim
    - Provide minimal `libdrm`-compatible stub (`drm_fd` → AetherOS gpu handle)
    - Deliver `libGL.so` + `libEGL.so` into initrd at `/lib/`
  - **Subtasks:**
    - [ ] **6.1.3a** Build Mesa with `--with-gallium-drivers=v3d --disable-glx --enable-osmesa`
    - [ ] **6.1.3b** Write `drm_shim.c` — map DRM ioctls to `SYS_GPU_*` syscall numbers
    - [ ] **6.1.3c** Link Mesa against AetherOS libc (`libaether`) + validate symbol resolution
    - [ ] **6.1.3d** Smoke-test: `glClear` + `glFlush` → pixels appear in framebuffer
- [ ] **6.1.4** Enable OpenGL ES 3.1 for applications — depends on 6.1.3
  - **Approach:** Add `#include <GLES3/gl31.h>` path to userspace build; provide `libGLESv2.so` symlink to Mesa
  - **Subtasks:**
    - [ ] **6.1.4a** GLES 3.1 smoke-test app (`userspace/apps/gl_test/main.c`): triangle on screen
    - [ ] **6.1.4b** Expose `SYS_GPU_SWAP` (905) — presents rendered BO to framebuffer (no tearing)
    - [ ] **6.1.4c** GLES-accelerated `gpu_glass_panel()` variant using fragment shader blur
    - [ ] **6.1.4d** Port desktop compositor blur to GLES path; benchmark vs TFU path
- [x] **6.1.5** GPU-accelerate compositor blur effects
  - TFU register offsets corrected to match Linux kernel V3D driver reference (0xe00 range)
  - `tfu_downsample_2x()` — kicks TFU job, polls V3D_HUB_INT_TFUC for completion
  - `v3d_tfu_blur()` — iterative TFU downsample chain (1–3 passes based on radius) + software bilinear upsample
  - `sw_bilinear_upsample()` — fixed-point 8-bit fractional bilinear upsample for final reconstruction
  - `v3d_blur()` hardware path now calls `v3d_tfu_blur()` on Pi 4; falls back to software on failure or QEMU
- [ ] **6.1.6** H.264/H.265 hardware decode via V3D
  - **Approach:** Use VideoCore firmware MMAL/HVS pipeline; bypass Mesa for decode-only path
  - **Subtasks:**
    - [ ] **6.1.6a** `mailbox_get_firmware_revision()` — confirm firmware supports MMAL
    - [ ] **6.1.6b** Allocate MMAL component via property mailbox, wire to framebuffer overlay
    - [ ] **6.1.6c** `kernel/drivers/gpu/mmal.c` — MMAL component lifecycle (create/connect/destroy)
    - [ ] **6.1.6d** Userspace `video_decode(path, x, y, w, h)` API — plays file into screen region
    - [ ] **6.1.6e** Media player app (`userspace/apps/aether_media/main.c`)
