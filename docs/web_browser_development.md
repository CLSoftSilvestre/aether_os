...
## Phase 7.2 — FreeType 2 & Font Integration

**Duration:** 2–3 weeks  
**Output:** `vendor/freetype2/`, `assets/fonts/`

Text rendering is a first-class requirement. FreeType renders TrueType/OpenType fonts to bitmaps.

| Task | Description |
|---|---|
| 7.2.1 | **FreeType 2.13.x** port — disable module autoconf; enable: sfnt, truetype, smooth rasterizer, cff |
| 7.2.2 | Bundle **Noto Sans** (variable weight) + **Noto Sans Mono** as static blobs in `assets/fonts/` |
| 7.2.3 | `aether_font.h/c` — FT_Library init; render_glyph(codepoint, size, color) → AetherOS framebuffer |
| 7.2.4 | Integrate with Lumina — replace existing bitmap font with FreeType in `gfx.c` |
| 7.2.5 | Integration test: render "Hello World" in 3 font sizes and a non-Latin script (Greek/Japanese) |

> **Note on HarfBuzz:** HarfBuzz (complex text shaping: Arabic, Hebrew, Devanagari) is deferred to a later iteration. Latin text works without it.

---

## Phase 7.3 — NetSurf Support Libraries

**Duration:** 3–4 weeks  
**Output:** `vendor/netsurf-libs/` (6 sub-libraries)

NetSurf depends on its own set of pure-C support libraries, maintained by the NetSurf project.

| Task | Library | Purpose |
|---|---|---|
| 7.3.1 | **libparserutils** | Character encoding, input stream processing |
| 7.3.2 | **libwapcaplet** | String interning (reduces memory in DOM) |
| 7.3.3 | **libhubbub** | HTML5 conformant parser (outputs tree-building events) |
| 7.3.4 | **libdom** | W3C DOM implementation; depends on libhubbub + libwapcaplet |
| 7.3.5 | **libcss** | CSS2.1/3 parser and selection engine |
| 7.3.6 | **libnsbmp / libnsgif / libnsutils** | BMP/GIF decoders + utility functions |
| 7.3.7 | Build all with `aarch64-none-elf` toolchain; validate no libc calls outside libaether_posix |
